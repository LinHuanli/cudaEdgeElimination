...
M4.3b3a 已在不改变 verifier 和文件格式的前提下加入主机 BFS 工作图、CUDA 原子 continuation counters 与单-block device-persistent queue，详见 [GPU wavefront](09_Hamilton_Tutte_GPU_Wavefront.md)。M4.3b3b1 又加入经 CPU 全量认证的 point/end [GPU path append](10_Hamilton_Tutte_GPU_Path_Append.md)，M4.3b3b2b2b1 已能写出完整规范 child edge SoA。仍需按深度/路径数/reply 数分桶、批量叶调用和 CPU long-tail 队列；完成后才能把 HT 删除接入不可变 epoch 的确定性 commit。
