...
3,008 条，删除 94,453 条，删除率 96.9136%。clean commit 上 7 次进程端到端
wall 中位数为 9.23 秒；GPU 主求解 wall 中位数为 6.582 秒。官方最优 tour
...
|---|---:|---:|---:|---:|---:|
| 作者 `KH -Jq` 单轮 | 12,914 | 94.89 s | 3,008 | 9.23 s | 更强且 `10.28x` |
| 作者 KH-HS/CUDA-JV 固定点 | 4,016 | 99.68 s | 3,008 | 9.23 s | 更强且 `10.80x` |
| 旧 FGPU 深 HT one-shot | 3,239 | 3,201.08 s | 3,008 | 9.23 s | 更强且 `346.81x` |

这里的 `10.28x/10.80x` 使用 `/usr/bin/time` 的完整进程 wall，而不是只取
...
|---:|---:|---:|---:|---:|
| 1 | 6.582 s | 2.346 s | 9.270 s | 9.36 s |
| 2 | 6.528 s | 2.299 s | 9.152 s | 9.22 s |
| 3 | 6.551 s | 2.408 s | 9.261 s | 9.33 s |
| 4 | 6.610 s | 2.283 s | 9.164 s | 9.23 s |
| 5 | 6.312 s | 2.340 s | 9.028 s | 9.09 s |
| 6 | 6.638 s | 2.306 s | 9.232 s | 9.30 s |
| 7 | 6.588 s | 2.273 s | 9.146 s | 9.21 s |
| **中位数** | **6.582 s** | **2.306 s** | **9.164 s** | **9.23 s** |

只用 GPU solve wall 计算，相对作者单轮/固定点分别为 14.42x/15.14x；这两项
只用于定位剩余 CPU 审计成本，不作为端到端主结论。

旧快速链为 8,015 条、4.872 秒。当前版本少保留 62.47% 的边并真正跑到
Quick-HS 固定点，但可信进程 wall 是其 1.89 倍。因此若下游只要求约 8k 边，
...
- CPU 后置审计仍占 trusted total 中位数约 2.31 秒（约 25.2%）。后续可优化
  verifier，但不能删除仓库规定的独立信任边界。

正式原始记录位于（Git 忽略的）
`artifacts/pcb442-fgpu-resident-20260904T055607Z-4091585/`；其
`benchmark.manifest` 绑定 clean commit `90c9c8a`、输入哈希、GPU UUID、
7 次测量和输出哈希。
