...
M4.3b3a 把递归 DFS 的求值顺序改为显式工作图，验证 AND–OR continuation 可以在 GPU 上稳定传播。主机仍负责工作图所有权和 leaf proof；M4.3b3b1 已把 point/end reply 的路径冲突标记加入 GPU，M4.3b3b2a 又加入多中心 Hamilton 邻边对 count/write。CPU 保留规范子状态生成，并完整复核两类 CUDA 输出。这个边界隔离最容易出错的 continuation 与路径语义，并为后续设备端状态写出和批量叶提供可差分基线。

`ht-prove --scheduler wavefront --reply-backend auto|cpu|cuda --path-append-backend auto|cpu|cuda --propagation-backend auto|cpu|cuda` 使用该路径。三个后端独立选择；它生成与 DFS 相同的 `CUDAEE_HT_RECURSIVE_PROOF_V1`，`ht-verify` 不区分证书来自哪一种调度器。
...
2. 未解决且未达深度上限时批量枚举 point 中心的 Hamilton replies，再按规范顺序生成 point moves，必要时生成 end moves；
...
- 固定 point CLI 的全 CUDA 候选路径产生 34 states、18 moves、84 replies，峰值 frontier 为 27；point/end append 为 9 batches/84 tasks，Hamilton replies 为 9 batches/16 centers/46 surviving pairs，压缩后是 4 节点可重放 proof；
- 空闲 RTX 4000 Ada 上完整 Hamilton–Tutte 单元测试经 compute-sanitizer memcheck 为 0 error，覆盖 persistent queue 与 path-append kernel。

## M4.3b3b 后续

M4.3b3b1 已完成 device-persistent 完成队列和批量 path 冲突标记，具体契约见 [GPU path append](10_Hamilton_Tutte_GPU_Path_Append.md)；M4.3b3b2a 已完成 Hamilton reply count/write，见 [GPU Hamilton replies](11_Hamilton_Tutte_GPU_Hamilton_Replies.md)。下一阶段迁移 end reply 提取和规范子状态 SoA，并加入按 `(depth,path_count,reply bucket)` 分桶的 leaf batches、多 block 调度和 CPU long-tail。完成 CPU/GPU 无截断差分、显存峰值与收益门禁后，才设计 HT proof sidecar 与不可变 epoch 的确定性提交。
