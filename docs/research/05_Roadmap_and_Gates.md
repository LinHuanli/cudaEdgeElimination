...
交付 epoch 读写、C API solve、残差、对偶裁剪、定点精确模型下界，以及按稳定列/行身份投影的 PDLP warm start。门禁：手算小 LP 目标与证书一致；失败状态不能产生删除授权；重复运行的证书分子/分母一致；身份或覆盖率不足时冷启动。均已完成。
...
- M3.1 删除强度：稳定列/行身份、覆盖率门禁和 cuOpt primal/dual warm start 基础设施已完成；迭代补列或稳定化后与原 QSopt 路径的安全删除结果等价，并导出可比边集；后半段待完成。
