...
M4.2 继续实现 3/4/5-opt 叶 witness、困难叶 CPU fallback、HS `c,d` AND–OR 搜索和 proof 重放。只有整条证书链完成后才把 HS 候选接入 epoch commit。
...
当前已交付 M0、M1、M2、M3 安全桥接和 M4.1。M3.1 与 M4.2 未完成时必须在状态清单中标为 pending，不能用合法但偏弱的下界或组合表替代删除强度与 HS 实现。
