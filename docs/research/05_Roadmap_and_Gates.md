...
M4.3b3b2b2b2b 继续扩展一般多 deletion-set leaf 游标，加入设备快照缓存、明确的 GPU/CPU long-tail 阈值和多 block 队列。CPU/GPU 对完整小实例真值及证书必须一致；之后才把验证成功的 HT 候选接入 epoch commit。
