...
后续已加入[设备快照/模板与 workspace 驻留缓存](18_Hamilton_Tutte_GPU_Leaf_Cache.md)和 [GPU/CPU long-tail](19_Hamilton_Tutte_CPU_Long_Tail.md)；路径规范对象、outside/inside 枚举及 witness 构造仍在主机。接下来加入多 block continuation。HT epoch commit 继续保持待实现。
