...
当前缓存为线程本地、同步 kernel 的正确性基线，没有跨线程共享或异步 stream 生命周期。后续 GPU/CPU long-tail 与 [cooperative multi-block continuation](20_Hamilton_Tutte_Multi_Block_Continuation.md)均已保持完整 proof 字节等价；下一步是 HT epoch commit。
