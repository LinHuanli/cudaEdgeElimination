...
`106.344 ms`、prepare `85.654 ms` 和 setup `57.806 ms`。下一步先单独计量哈希建表、唯一
task 搬运、距离表和 row scoring；只有画像证明去重管理仍占显著比例，才考虑上游规范 task id
或更紧凑的批内索引。端口互异双 scorer 路径继续保持否决状态。
