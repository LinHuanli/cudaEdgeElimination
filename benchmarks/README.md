...
V3 报告将工作图总时间进一步拆成 leaf、path-append、Hamilton reply 与 end reply，并单列传播、proof 抽取、三层 CPU 重放和最终 commit；它还记录 leaf frontier/bucket/cost batch 数与融合开关。`work_graph_ms` 是包含式总量，不能再与其四个子阶段相加。summary 中的 `host_build_residual_ms` 已按包含关系扣除四个子阶段。
