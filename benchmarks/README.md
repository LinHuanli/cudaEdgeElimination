...
V4 报告将工作图总时间进一步拆成 leaf、path-append、Hamilton reply 与 end reply，并单列传播、proof 抽取、三层 CPU 重放和最终 commit；它还记录 leaf frontier/bucket/cost batch 数、融合开关，以及 leaf 内部 setup/cursor/cost/consume/apply/verifier 计时。`work_graph_ms` 与 `leaf_ms` 都是包含式总量，不能再与各自子阶段相加。summary 中的两个 residual 已按包含关系扣除对应子阶段。
