...
V7 报告将工作图总时间进一步拆成 leaf、path-append、Hamilton reply 与 end reply，并单列传播、proof 抽取、三层 CPU 重放和最终 commit；它还记录 leaf frontier/bucket/cost batch 数、融合开关、leaf 内部 setup/cursor/cost/consume/apply/verifier、CPU 精确 cost-matrix 认证，以及 Hamilton reply 的 validation/CPU enumerate/CUDA evaluate/compare。`work_graph_ms`、`leaf_ms`、`leaf_cost_evaluate_ms`、`leaf_cursor_consume_ms` 和 `hamilton_reply_ms` 都是包含式总量，不能再与各自子阶段相加。V9 summary 要求四路 leaf 与 Hamilton reply 的规范工作计数完全一致；residual 已按对应包含关系扣除。
