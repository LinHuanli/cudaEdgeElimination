...
- 主机 BFS 工作图、device-persistent CUDA continuation 传播、跨父状态 Hamilton/end reply count/write、point/end path-append、规范 child edge SoA、增量 k-opt leaf cost block 融合与 GPU 驻留快照/模板缓存，并由 CPU 完整差分复核；
- CPU 单元测试、CUDA 差分测试入口和 pr299 集成脚本。

尚未完成的研究项（明确的 CPU long-tail、多 block continuation 与 epoch commit、cuOpt 退化对偶稳定化与精确定价后边集导出）会显式安全回退，详见 [研究路线图](docs/research/05_Roadmap_and_Gates.md)。精确困难叶有 18 个 block 的硬上限，超限只返回 `unresolved`。递归 HT proof 尚未接入删边；`lp-solve` 本身也永不删除边，只有 Concorde 桥接路径经过完整图精确定价后才产生下界授权。
