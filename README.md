...
- `fgpu-elim resident` 的单卡全常驻主链：CUDA FP64 有向舍入 Geometry、device-resident degree-box PDLP、exhaustive JV 与 Quick-HS 固定点；CPU 只在搜索结束后做不反馈设备的精确安全审计。pcb442 七次端到端中位数为 11.88 秒、最终 3,008 条边，详见 [全常驻基准](docs/research/68_FGPU_单卡全常驻实现与端到端基准.md)。
